...
struct GoldenFallbackTests {
  ...
  func multiRuleFallbackWithPriorityGolden() throws {
    ...
    let result = try runTestPipeline(bytes: bytes, artifact: FallbackFixtures.multiRuleFallbackWithPriority())
    #expect(
      renderSnapshot(result)
        == """
  ...
  func mixedFastAndFallbackGolden() throws {
    ...
    let result = try runTestPipeline(bytes: bytes, artifact: FallbackFixtures.mixedFastAndFallback())
    #expect(
      renderSnapshot(result)
        == """
