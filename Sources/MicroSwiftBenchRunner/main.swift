...
func benchM4GPU(bytes: [UInt8], runtime: ArtifactRuntime, warmup: Int, measure: Int) -> BenchResult
{
  ...
  return Device.withDefaultDevice(.gpu) {
    ...
    var lastResult: DeviceLexResult?
    for _ in 0..<measure {
      lastResult = TensorLexer.lexPageDevice(
        compiledPage: compiledPage,
        artifact: runtime,
        options: gpuOptions
      )
    ...
    let tokenCount = lastResult?.hostRowCount() ?? 0
    ...
    return BenchResult(
      ...
      tokensPerIteration: Int(tokenCount)
    )
  }
}
...
do {
  ...
  for i in 0..<m3Results.count {
    ...
    let cpuStr =
      m4Results[i].map { String(format: "%.2fx", $0.bytesPerSecond / m3Results[i].bytesPerSecond) }
      ?? "—"
