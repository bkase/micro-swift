...
public func runBenchmark(
  ...
  let benchmarkBytes =
    config.mode == .error
    ? makeErrorPathBytes(from: bytes, seed: config.seed)
    : bytes
  ...
  func executeSingleRun() -> (tokenCount: Int, errorSpanCount: Int) {
    ...
    let selected = greedyNonOverlapSelect(winners: fastWinners, validLen: validLen)
  ...
  for _ in 0..<measuredIterations {
    let iteration = executeSingleRun()
...
private func makeErrorPathBytes(
  ...
  for idx in output.indices {
    // Use high-bit bytes that are unlikely to match any rule
    output[idx] = UInt8(rng.next() & 0xFF)
  }
